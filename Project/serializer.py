...
class InterfaceByIDSerializer(serializers.ModelSerializer):
    #指定母表关联字段应该子表名默认 子表模型类名小写_set
    project2s_set = Project2Serializer(read_only=True, many=True)
    class Meta:
        model = Projects
        # 如果添加一个Projects不存在的字段，必须包裹在fields中
        fields = ('id', 'name','project2s_set')
